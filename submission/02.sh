...
VOUT_0=$(bitcoin-cli -regtest decoderawtransaction $transaction | jq -r ".vout | .[0] | .n")
VOUT_1=$( bitcoin-cli -regtest decoderawtransaction $transaction | jq -r ".vout | .[1] | .n")
ADDRESS="2MvLcssW49n9atmksjwg2ZCMsEMsoj3pzUP"

TOTAL_AMOUNT=$(bitcoin-cli -regtest decoderawtransaction $transaction | jq -r '.vout | .[] | .value' | awk '{s+=$1} END {print int(s*100000000 + 0.5)}')

AMOUNT=0.2

NEXT_BLOCK=$( echo "14*24*60/10" | bc)
BLOCK_TARGET=$(echo "$NEXT_BLOCK+ 25" | bc)


bitcoin-cli -regtest -named createrawtransaction inputs='''[{"txid":"'$TXID'","vout":'$VOUT_0'},{"txid":"'$TXID'","vout":'$VOUT_1'}]''' outputs='''[{"'$ADDRESS'":'$AMOUNT'}]''' locktime=$BLOCK_TARGET
